...
export default function Header() {
  const { firebaseUser, userProfile, userRole, logout } = useAuth();
            ...
            <DropdownTrigger>
              <div className="flex items-center gap-3 cursor-pointer">
                <div className="hidden sm:block text-right">
                  <p className="text-white font-semibold text-sm">{userProfile?.displayName || 'Usuario'}</p>
                  <p className="text-blue-200 text-xs capitalize">{userRole || 'user'}</p>
                </div>
                <Avatar
                  isBordered
                  as="button"
            ...
            <DropdownMenu aria-label="User Actions" variant="flat">
              ...
              {userProfile?.balance !== undefined && userProfile?.balance !== null && (
                <DropdownItem key="balance">
                  Saldo: S/ {userProfile.balance.toFixed(2)}
                </DropdownItem>
              )}
              {userProfile?.level && (
