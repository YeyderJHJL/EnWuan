...
export default function SurveyDetail() {
  ...
  // Load survey and check if already submitted
  useEffect(() => {
    const loadSurvey = async () => {
      try {
        ...
        if (firebaseUser?.uid) {
          try {
            const checkResponse = await submissionsService.checkUserSubmission(surveyId);
            if (checkResponse.data?.hasSubmitted) {
              setAlreadySubmitted(true);
            }
          } catch (err) {
            console.log('Could not check previous submission:', err);
          }
        }
  ...
  const handleSubmit = async () => {
    try {
      ...
      const errorMessage = err.response?.data?.message || 'Error al enviar respuestas';
      setError(errorMessage);
      // If error is about already submitting, set the flag
      if (errorMessage.includes('Ya has respondido')) {
        setAlreadySubmitted(true);
      }
    } finally {
      setSubmitting(false);
  ...
  // Already submitted - show message and redirect option
          ...
          <Card className="bg-gradient-to-br from-amber-50 to-orange-50 border-2 border-amber-300">
            <CardBody className="p-8">
              <div className="text-center">
                ...
                <h2 className="text-3xl font-bold text-amber-900 mb-4">Encuesta Completada</h2>
                <p className="text-lg text-amber-800 mb-6">
                  Ya has respondido esta encuesta. Cada usuario solo puede responder una vez.
                </p>
                <p className="text-sm text-amber-700 mb-8">
                  Gracias por tu participación y ayudar a mejorar nuestros servicios.
                </p>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  ...
                  <Button
                    ...
                    variant="bordered"
                    className="border-amber-600 text-amber-600 font-bold"
                    size="lg"
                  >
                    Mi Dashboard
                  </Button>
